...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements RoleService {
    ...
    @Override
    public Role getByBizId(String bizId) {
        return Optional.ofNullable(roleMapper.selectByBizId(bizId)).orElseThrow(() -> new NotFoundException("角色不存在"));
    ...
    public void update(Role role) {
        int affectedRows = roleMapper.updateByBizId(role);
    ...
    public void deleteByBizId(String bizId) {
        int affectedRows = roleMapper.deleteByBizId(bizId);
        if (affectedRows <= 0) {
