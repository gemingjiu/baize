...
import com.baize.system.mapper.SysUserMapper;
...
public class SysUserServiceImpl implements ISysUserService {
    ...
    private void initSysUser(SysUser info) {
    ...
    public boolean checkUserNameUnique(SysUser user) {
        ...
        if (StringUtils.isNotNull(info) && !StringUtils.equals(info.getId(), userId)) {
    ...
    public void checkUserAllowed(SysUser user) {
        if (StringUtils.isNotNull(user.getId()) && user.isAdmin()) {
            throw new BaseException(SERVICE_EXCEPTION, "不允许操作超级管理员用户");
        }
    }
    ...
    public void checkUserDataScope(String userId) {
        if (!SysUser.isAdmin(SecurityUtils.getId())) {
            ...
            if (StringUtils.isEmpty(users)) {
                throw new BaseException(SERVICE_EXCEPTION, "没有权限访问用户数据！");
            }
        }
    }
