...
const genList1 = (current: number, size: number) => {
...
let tableListDataSource = genList1(1, 100);
...
function getRule(req: Request, res: Response, u: string) {
  ...
  let dataSource = [...tableListDataSource].slice(
    ((current as number) - 1) * (size as number),
    (current as number) * (size as number),
  );
  if (params.sorter) {
  ...
  let finalsize = 10;
  if (params.size) {
    finalsize = parseInt(`${params.size}`, 10);
  }
