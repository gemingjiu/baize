...
import com.baize.common.core.enums.DataStatus;
...
public class SysLoginService {
    ...
    public LoginUser login(String username, String password) {
        ...
        if (password.length() < UserConstants.PASSWORD_MIN_LENGTH
            ...
            throw new SystemException(SERVICE_EXCEPTION, "用户密码不在指定范围");
        }
        // 用户名不在指定范围内 错误
        if (username.length() < UserConstants.USERNAME_MIN_LENGTH
            || username.length() > UserConstants.USERNAME_MAX_LENGTH) {
            throw new SystemException(SERVICE_EXCEPTION, "用户名不在指定范围");
        }
        // IP黑名单校验
        String blackStr = ConvertUtils.toStr(CacheService.getCacheObject(CacheConstants.LOGIN_BLACKLIST_PREFIX));
        if (IpUtils.isMatchedIp(blackStr, IpUtils.getIpAddr())) {
            throw new SystemException(SERVICE_EXCEPTION, "很遗憾，访问IP已被列入系统黑名单");
        }
        // 查询用户信息
        Response<LoginUser> userResult = remoteUserService.getUserInfo(username, SecurityConstants.INNER);

        if (StringUtils.isNull(userResult) || StringUtils.isNull(userResult.getData())) {
            throw new SystemException(SERVICE_EXCEPTION, "登录用户：" + username + " 不存在");
        }

        if (Response.FAIL == userResult.getCode()) {
            throw new SystemException(SERVICE_EXCEPTION, userResult.getMsg());
        }

        LoginUser userInfo = userResult.getData();
        SysUser user = userResult.getData().getSysUser();
        if (DataStatus.DELETED.getStatus().equals(user.getStatus())) {
            throw new SystemException(SERVICE_EXCEPTION, "对不起，您的账号：" + username + " 已被删除");
        }
        if (DataStatus.DISABLED.getStatus().equals(user.getStatus())) {
            throw new SystemException(SERVICE_EXCEPTION, "对不起，您的账号：" + username + " 已停用");
        }
    ...
    public void register(String username, String password) {
        ...
        if (username.length() < UserConstants.USERNAME_MIN_LENGTH
            || username.length() > UserConstants.USERNAME_MAX_LENGTH) {
            throw new SystemException(SERVICE_EXCEPTION, "账户长度必须在2到20个字符之间");
        }
        if (password.length() < UserConstants.PASSWORD_MIN_LENGTH
            || password.length() > UserConstants.PASSWORD_MAX_LENGTH) {
            throw new SystemException(SERVICE_EXCEPTION, "密码长度必须在5到20个字符之间");
        }
        ...
        if (Response.FAIL == registerResult.getCode()) {
            throw new SystemException(SERVICE_EXCEPTION, registerResult.getMsg());
        }
    }
}
